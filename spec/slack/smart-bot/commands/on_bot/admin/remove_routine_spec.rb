...
RSpec.describe SlackSmartBot, "remove_routine" do
  describe "on channel bot" do
    ...
    it "deletes the routine if admin user" do
      ...
      sleep 2
      send_message "remove routine example", from: user, to: channel
      sleep 1
      expect(bufferc(to: channel, from: :ubot).join).to match(/The routine \*`example`\* has been removed./)
      sleep 1
      clean_buffer()
      sleep 3
    ...
    it "accepts on 'delete'" do
      send_message "delete routine example", from: user, to: channel
      expect(buffer(to: channel, from: :ubot).join).not_to eq("")
    end
    it "accepts on 'kill'" do
      send_message "delete routine example", from: user, to: channel
      expect(buffer(to: channel, from: :ubot).join).not_to eq("")
    end

    it "doesn't allow to delete a routine if not admin" do
      sleep 1
      send_message "delete routine example", from: :user1, to: channel
      expect(buffer(to: channel, from: :ubot).join).to match(/Only admin users can delete routines/)
    end
  end
  ...
  describe "on master channel" do
    ...
    it "can be called" do
      send_message "delete routine example", from: user, to: channel
      expect(bufferc(to: channel, from: :ubot).join).to match(/Only admin users can delete routines/)
    end
  end
  ...
  describe "on extended channel" do
    it "doesn't respond" do
      send_message "!delete routine example", from: :uadmin, to: :cext1
  ...
  describe "on external channel not extended" do
    it "doesn't respond to external demand" do
      command = "remove routine example"
