  ...
  def hi_bot(user, dest, from, display_name)
    if @status == :on
      ...
      if Thread.current[:using_channel]!=''
        respond "If you want to change bot channel rules call `use #CHANNEL` or `stop using rules from <##{Thread.current[:using_channel]}>` to stop using rules from this channel."
      end
