  ...
  describe "add memo team" do
    describe "on external channel" do
      ...
      it "is possible to add a memo if you are a master admin" do
        ...
        expect(bufferc(to: channel, from: :ubot).join).to match(/:memo:\s+\d\d\d\d\/\d\d\/\d\d:\s+some\stext\s\(marioruizs\s\d+\)/)
      end
      ...
      it "is possible to add a note" do
        ...
        expect(buffer(to: channel, from: :ubot).join).to match(/The memo has been added to \*example\* team/)
        expect(bufferc(to: channel, from: :ubot).join).to match(/:abc:\s+\d\d\d\d\/\d\d\/\d\d:\s+some\stext\s\(smartbotuser1\s\d+\)/)
      end

      it "is possible to add an issue" do
        send_message "add issue to example team : some text", from: :user1, to: channel
        expect(buffer(to: channel, from: :ubot).join).to match(/The memo has been added to \*example\* team/)
        expect(bufferc(to: channel, from: :ubot).join).to match(/:hammer:\s+\d\d\d\d\/\d\d\/\d\d:\s+some\stext\s\(smartbotuser1\s\d+\)/)
      end

      it "is possible to add a task" do
        send_message "add task to example team : some text", from: :user1, to: channel
        expect(buffer(to: channel, from: :ubot).join).to match(/The memo has been added to \*example\* team/)
        expect(bufferc(to: channel, from: :ubot).join).to match(/:clock1:\s+\d\d\d\d\/\d\d\/\d\d:\s+some\stext\s\(smartbotuser1\s\d+\)/)
      end

      it "is possible to add a feature" do
        send_message "add feature to example team : some text", from: :user1, to: channel
        expect(buffer(to: channel, from: :ubot).join).to match(/The memo has been added to \*example\* team/)
        expect(bufferc(to: channel, from: :ubot).join).to match(/:sunny:\s+\d\d\d\d\/\d\d\/\d\d:\s+some\stext\s\(smartbotuser1\s\d+\)/)
      end

      it "is possible to add a bug" do
        send_message "add bug to example team : some text", from: :user1, to: channel
        expect(buffer(to: channel, from: :ubot).join).to match(/The memo has been added to \*example\* team/)
        expect(bufferc(to: channel, from: :ubot).join).to match(/:bug:\s+\d\d\d\d\/\d\d\/\d\d:\s+some\stext\s\(smartbotuser1\s\d+\)/)
      end
