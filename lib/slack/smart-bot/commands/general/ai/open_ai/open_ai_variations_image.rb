                ...
                begin
                  ...
                  if (!File.exist?("#{config.path}/tmp/#{user.name}_#{@ai_open_ai_image[user.name].object_id}.png") and (files.nil? or files.size != 1))
                    ...
                    if !files.nil? and files.size == 1
                      ...
                      success, res = SlackSmartBot::AI::OpenAI.send_image_variation(@ai_open_ai[user.name][:dall_e].client, image, variations, size: @ai_open_ai[user.name][:dall_e][:image_size])
                    else
                      success, res = SlackSmartBot::AI::OpenAI.send_image_variation(@ai_open_ai[user.name][:dall_e].client, image, variations, size: @ai_open_ai[user.name][:dall_e][:image_size])
                    end
                    if success
